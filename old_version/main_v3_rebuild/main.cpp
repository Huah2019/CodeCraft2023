...
void moveRobots(std::vector<Robot> &robots)
{
  ...
  for (int i = 0; i < int(ids.size()); ++i)
  {
    ...
    {
      double mn = r.radar[j];
      for (int k = j, h = W; h; --h, k = (k + 1) % 360)
        mn = std::min(mn, r.radar[k]);
      if (mn > L)
      {
        canNotMoveAnyway = false;
        break;
      }
    }
    if (canNotMoveAnyway)
          ...
          {u - v.unit() * r.radius,
           u + v.unit() *
                   (r.radius +
                    std::min(v.len(),
                             (r.v.len() +
                              MAX_DELTA_lineSpeed_PER_FRAME[r.item > 0]) /
                                 FPS))});
    }
  }
