...
void updateMechanism()
{
  ...
  for (auto &r : mp.robots)
    if (r.isTransporter())
    {
      if (r.state != 2 || r.sell == -1 || r.item == 0)
      {
        r.rushBackward = -1;
        continue;
      }
      ...
      if (r.rushBackward == -1)
      {
        if (!p.isSafe && p.rushAble && mp.getEvalDis(r.p, p.id, 0, r.radius) <= 2)
          r.rushBackward = 0;
      }
      if (r.rushBackward == 0)
      {
        if (mp.getEvalDis(r.p, p.id, 0, r.radius) >= 1.5)
          r.rushBackward = 1;
        double angle = r.p.angleTo(p.p);
        double angleDis = normAngle(angle + PI - r.angle);
        if (angleDis < 0)
          angleDis += PI;
        int dir = angleDis / PI * 180;
        if (r.radar[dir] <= r.radar[dir] + 0.1)
          r.rushBackward = -1;
      }
    }
  ...
  //     bool flag = false;
  //     for (auto &f : mp.foes)
  //       if (r.p.dis(f.pos) <= r.radius + f.radius + 0.05)
  //       {
  //         double angle = normAngle(r.angle - r.p.angleTo(f.pos));
  //         if (abs(angle) <= PI / 3)
  //           flag = true;
  //         if (r.checkIsStop())
  //           flag = true;
