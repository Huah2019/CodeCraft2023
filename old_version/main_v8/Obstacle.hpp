...
struct Obstacle
{
  ...
  template <class T>
  static std::vector<std::pair<Pt, Pt>> getNearbyObstaclePointsToSegment(T &map,
                                                                         Pt p, int d)
  {
    ...
    for (int i = std::max(-1, x - d); i <= MAP_SIZE && i <= x + d; i++)
    {
      for (int j = std::max(-1, y - d); j <= MAP_SIZE && j <= y + d; j++)
      {
        if (i < 0 || j < 0 || i == MAP_SIZE || j == MAP_SIZE ||
            map[i][j] == '#')
        {
          D centerX = j * 0.5, centerY = (MAP_SIZE - 1 - i) * 0.5;
          os.push_back({Pt(centerX - 0.25, centerY - 0.25), Pt(centerX - 0.25, centerY + 0.25)});
          os.push_back({Pt(centerX - 0.25, centerY + 0.25), Pt(centerX + 0.25, centerY + 0.25)});
          os.push_back({Pt(centerX + 0.25, centerY + 0.25), Pt(centerX + 0.25, centerY - 0.25)});
          os.push_back({Pt(centerX + 0.25, centerY - 0.25), Pt(centerX - 0.25, centerY - 0.25)});
        }
