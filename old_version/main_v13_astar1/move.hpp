...
struct HalfplaneBuilder {
  ...
  HalfplaneBuilder(const Robot &robot, const std::vector<Obstacle> &os, const D th, const D thObst)
      : robot(robot), os(os), radius(robot.radius + SELF_EXR), th(th), thObst(thObst),
...
inline void moveRobots() {
  ...
  for (int i : ids) {
    auto &r = robots[i];
    int pid = r.getTarget();
    Pt makeWayTo = mp.makeWayTo(r.p, r.radius, segs,
                                r.isTransporter() && pid >= 0 ||
                                        r.isAttacker() && r.attackFoeId != -1
                                    ? 0.5
                                    : 1e9);
  ...
  for (Robot &rob : robots) {
    ...
    if (target != -2 && mp.getEvalDis(rob) <= 2)
