...
struct HalfplaneBuilder
{
  ...
  void handleRobots()
  {
    ...
    for (auto &o : rs)
    {
      ...
      D d2 = rp.len2(), sr = radius + R[o.item > 0] + 0.02, sr2 = sqr(sr);
...
inline std::pair<D, D> getNewArgs(const Robot &rob, const std::vector<Robot> &rs,
                                  const std::vector<Obstacle> &os)
{
  HalfplaneBuilder hp(rob, rs, os, rob.radius + 0.02, TIME_HORIZON,
                      TIME_HORIZON_OBSTACLE);
  Pt optV = hp.solve(std::fmin(MAX_FORWARD_SPEED[Map::instance().role],
                               rob.v.len() + MAX_DELTA_LINE_SPEED_PER_FRAME[rob.item > 0]));
  D angleDist = normAngle(atan2(optV.y, optV.x) - rob.angle);
  D absAngle = std::fabs(angleDist);
  D maxW = std::min(absAngle / TIME_STEP * 2, MAX_ROTATE_SPEED) * sgn(angleDist);
  D maxV = optV.len();
  D eV, eW;
