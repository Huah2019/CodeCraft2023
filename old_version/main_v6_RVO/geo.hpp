...
struct Pt
{
  ...
  friend D dot(const Pt &a, const Pt &b)
  {
...
inline Pt operator*(D a, const Pt &b)
{
  return b * a;
}
inline D cross(const Pt &a, const Pt &b)
{
...
{
  D oa = c.cross(d, a), ob = c.cross(d, b), oc = a.cross(b, c), od = a.cross(b, d);
  return sgn(oa) * sgn(ob) < 0 && sgn(oc) * sgn(od) < 0;
}
inline D distPL(Pt p, Pt a, Pt b) { return fabs(p.cross(a, b)) / (b - a).len(); }
...
{
  return !checkSS(a, b, c, d) ? std::min({distPS(a, c, d), distPS(b, c, d), distPS(c, a, b), distPS(d, a, b)}) : 0.0;
}
