...
constexpr D EXR = 0.02; // radius = robot.radius + EXR
...
struct HalfplaneBuilder {
  ...
  HalfplaneBuilder(const Robot &robot, const std::vector<Robot> &rs,
                   const std::vector<Obstacle> &os)
      : robot(robot), rs(rs), os(os) {
    radius = R[robot.item > 0] + EXR;
    lines.clear();
    ...
    for (auto &o : rs) {
      ...
      if (d2 > sr2) {
        ...
        if (dp > 0.0 && sqr(dp) > sr2 * w.len2()) {
          ...
          v = cross(rp, w) < 0.0 ? Pt(rp.x * leg - rp.y * sr,
                                             rp.y * leg + rp.x * sr)
                                        : Pt(-rp.x * leg - rp.y * sr,
                                             rp.x * sr - rp.y * leg);
          v = v / d2;
