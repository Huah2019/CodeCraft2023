...
public:
  ...
  void getMapId();
  void getMapType();
  void init(int, std::vector<std::string> &, bool);
  ...
  D getEvalDis(Robot &rob) { return getEvalDis(rob.p, rob.getTarget(), 0, rob.radius); }
};
...
struct Graph
{
  ...
  dijkstra(const std::vector<std::pair<int, D>> &s, int role)
  {
    ...
    while (!q.empty())
    {
      ...
      for (int i = deg[x]; i < deg[x + 1]; i++)
      {
        ...
        if (d[x].second != -1)
        {
          ...
          D angCost =
              ang / MAX_ROTATE_SPEED * MAX_FORWARD_SPEED[role];
          if (ang < PI / 6)
          {
            z += angCost * 0.1;
          }
