...
void processRadarInfo()
{
  ...
  auto isInObs = [&](Pt &p)
  {
    for (auto &obs : Obstacle::getNearbyObstaclePointsToSegment(mp.g, p, 1))
      if (isOnPS(p, obs.first, obs.second))
        return true;
    return false;
  };
  for (size_t i = 0; i < robots.size(); ++i)
  {
    ...
    for (int i = 0; i < 360; ++i)
    {
      ...
      if (isInObs(a) || isInObs(b) || isInObs(c))
        continue;
      ...
      if (isNotIn)
        es.push_back({center, Pt(0, 0), radius, 1 << r.id});
    }
  }
  // 每帧最大移动距离为7/50=0.14
  for (auto &e : es)
    for (auto &eOld : mp.enemys)
...
void init()
{
  ...
  {
    mp.init(0, g, true);
    mpFoe.init(1, g, false);
  }
  else
  {
    mp.init(1, g, true);
    mpFoe.init(0, g, false);
  }
...
void updTaskForAttacker()
{
  auto interfereFoe = [&](Robot &r)
  {
    ...
    if (mp.enemys.empty())
      return;
    double mndis = 1e9;
    for (int id = 0; id < mp.enemys.size(); ++id)
    {
      auto &e = mp.enemys[id];
      if (!(e.visitIt >> r.id & 1))
        continue;
      if (e.velocity.len() <= EPS)
        continue;
      double angle = normAngle(atan2(e.velocity.y, e.velocity.x) - e.pos.angleTo(r.p));
      if (e.pos.dis(r.p) < mndis) // 参数，距离敌方机器人的距离
      {
        mndis = e.pos.dis(r.p);
    ...
    if (r.isAttacker())
    {
      ...
      if (r.attackPlatId == -1)
      {
        ...
        for (int pid : pids)
        {
          ...
          if (r.attackPlatId == -1 || Random::randDouble(0.5, 1) > 1.0 * (curdis + 1) / (mndis + 1))
          {
            mndis = curdis;
            r.attackPlatId = pid;
          }
...
void work()
{
  while (std::cin >> frameID >> money)
  {
    ...
    // if (frameID % 100 == 0) {
    //   for (auto &r : mp.robots) {
    //     dbg(frameID, r.id, r.p, r.buy >= 0 ? mp.plats[r.buy].type : -1,
    //     r.sell >= 0 ? mp.plats[r.sell].type : -1);
    //   }
    // }
  }
}
