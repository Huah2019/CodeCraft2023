...
void Map::getRobotAndPlatformInfo()
{
  ...
  for (auto &p : plats)
  {
    p.rushAble = 0;
    const D DIST = 1.25;
    // -0.4 + 0.53 + 0.53 * 2 = 1.19
    for (int i = 0; i < 180; i++)
    {
      D ang = i * PI / 180;
      Pt v(DIST * std::cos(ang), DIST * std::sin(ang));

      if (queryReachability(p.p - v, p.p + v, 0.6))
      {
        p.rushAble = 2;
        break;
      }
  ...
  for (auto &p : plats)
    if (p.rushAble == 0)
    {
      const D DIST1 = 1.25, DIST2 = 0.72;
      // -0.4 + 0.53 + 0.53 = 1.19
      for (int i = 0; i < 360; i++)
      {
        D ang = i * PI / 180;
        Pt v(std::cos(ang), std::sin(ang));

        if (queryReachability(p.p - v * DIST1, p.p + v * DIST2, 0.55))
        {
          p.rushAble = 1 + (role == 0);
          break;
        }
