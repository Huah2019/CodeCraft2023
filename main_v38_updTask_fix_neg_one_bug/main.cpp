      ...
      if (r.isStop)
      {
        if (r.sell >= 0)
        {
          ...
          mp.plats[r.sell].willBuy ^= 1 << r.item;
               ...
               mp.plats[r.sell].unSafe && mp.plats[r.sell].rushAble == 0)
        swithTarget(r);
    }
...
void updTaskForTransporter8()
{
  ...
  for (auto &r : robots)
    if (r.isTransporter() && r.state == 2)
    {
      ...
      else if (r.sell == -1 || mp.plats[r.sell].unSafe && mp.plats[r.sell].rushAble == 0)
        swithTarget(r);
    }
}
    ...
    if (r.isAttacker())
    {
      ...
      else
      {
        ...
        for (int id = 0; id < mp.foes.size(); ++id)
        {
          ...
          if (mpFoe.plats[r.attackPlatId].rushAble == 2)
          {
...
void work()
{
  while (std::cin >> frameID >> money)
  {
    ...
    if (frameID >= 2500 && mp.role == 1)
    {
      Logger::instance().write("FrameID", frameID);
      for (auto &r : mp.robots)
        Logger::instance().write(r.id, r.state, r.buy, r.sell, r.vitualTargetPID, r.pid);
    }
