...
inline void moveRobots() {
  ...
  for (int i : ids) {
    ...
    Pt makeWayTo = mp.makeWayTo(
        ...
        r.isTransporter() && pid >= 0 ||
                r.isAttacker() && (r.attackBuy123 >= 0 || r.attackFoeId != -1)
            ? 0.5
            : 1e9);
  ...
  for (Robot &rob : robots) {
    ...
    if (target != -2 && rob.p.dis(mp.plats[target].p) <= 6 &&
  ...
  for (Robot &rob : robots) {
    ...
    if (rob.isTransporter()) {
      for (auto &f : mp.foes) {
        Pt v = f.pos - rob.p;
        if (v.len() <= 1.5 && std::fabs(normAngle(std::atan2(v.y, v.x) - rob.angle)) <= PI / 4) {
          rob.lineSpeed = 100;
        }
     }
    }
