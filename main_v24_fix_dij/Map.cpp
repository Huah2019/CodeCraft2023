...
void Map::getMapType() {
  ...
  for (int i = 0; i < MAP_SIZE; ++i)
    for (int j = 0; j < MAP_SIZE; ++j) {
      int ii = j * 2 + 1, jj = (MAP_SIZE - 1 - i) * 2 + 1;
      int u = ii * GN + jj;
      if (g[i][j] >= '1' && g[i][j] <= '9')
        color[getLeader(u)] |= 1;
  ...
  auto cal = [&](std::string &s) {
    ...
    int ans = 0;
    for (int i = 0; i < MAP_SIZE; ++i) {
      for (int j = 0; j + sz <= MAP_SIZE; ++j) {
        int ii = j * 2 + sz, jj = (MAP_SIZE - 1 - i) * 2 + 1;
        int u = ii * GN + jj;
    ...
    for (int i = 0; i + sz <= MAP_SIZE; ++i) {
      for (int j = 0; j < MAP_SIZE; ++j) {
        int ii = j * 2 + 1, jj = (MAP_SIZE - i) * 2 - sz;
        int u = ii * GN + jj;
        if (!color[getLeader(u)])
          continue;
        bool ok = true;
