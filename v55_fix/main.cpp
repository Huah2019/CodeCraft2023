...
void initRobotType()
{
  ...
  for (int type : types)
  {
    ...
    {
      bool flag = true;
      for (int pid : foePlatsOnBeltline7[type])
        if (mpFoe.plats[pid].rushAble == 2)
        {
          flag = false;
          break;
        }
  ...
  if (mp.type == 2)
  {
    for (auto &r : mp.robots)
      if (r.type != 1)
      {
        ...
        {
          int nextime = bplat.getNextTime(r.unSafe, r.p);
...
void sellItem()
{
  for (Robot &r : mp.robots)
    if (r.state == 2 && r.sell >= 0 && r.pid == r.sell)
    {
      bool ok = mp.plats[r.pid].buyItem(r.item);
      if (ok)
      {
        auto &p = mp.plats[r.pid];
        p.unSafeFrame = 50;
        p.tryApply = 0;
        p.disableCons = 500;
        p.disable = 0;
...
{
  for (Robot &r : mp.robots)
    if (r.state == 1 && r.buy >= 0 && r.pid == r.buy)
    {
      bool flag = true;
...
void delTask(vector<Robot> &robots, vector<int> &num)
{
  for (auto &r : robots)
    if (r.state == 2 && (r.isStop || r.sell != -1 && (mp.plats[r.sell].disable || mp.plats[r.sell].unSafe && !mp.plats[r.sell].rushAble)))
    {
      if (r.sell >= 0)
      {
        auto &splat = mp.plats[r.sell];
        ...
        if ((splat.willBuy | splat.alreadyBuy) == splat.needBuy)
          --num[splat.type];
        if ((splat.willBuy | splat.alreadyBuy) == splat.needBuy && splat.type >= 4 && splat.type <= 6)
          --num456[splat.type];
...
void switchTask(vector<Robot> &robots, vector<int> &num)
{
  ...
  for (auto &r : robots)
    if (r.isTransporter() && r.state == 2 && !r.isStop && r.sell == -1)
    {
      swithTarget(r);
    }
...
void updTaskForTransporter6()
{
  ...
  while (it--)
  {
    ...
    {
      auto &rob = robots[i];
      if (!rob.isTransporter() || rob.state == 2 && !transmode || rob.isStop)
        ok[i] = true;
      if (ok[i])
        continue;
      if (rob.state == 2)
      ...
      else
      {
        for (Platform &bplat : mp.plats)
        {
          int nextime = bplat.getNextTime(rob.unSafe, rob.p);
          ...
          for (int area : bplat.validAreas)
          {
            ...
            for (Platform &splat : mp.plats)
            {
              if (splat.canBuy(bplat.sell))
              {
                ...
                if (canMake7 && bplat.type >= 4 && bplat.type <= 6 && splat.type == 9 && num[bplat.type] <= 1 && sellDisTo7[bplat.id] <= 1e9 - 100)
                  baseCof -= 666;
                Score *= baseCof;
    ...
    if (r.isAttacker())
    {
      ...
      else
      {
        ...
        for (int id = 0; id < mp.foes.size(); ++id)
        {
          ...
          if (mpFoe.plats[r.attackPlatId].rushAble)
          {
    ...
    for (auto &r : mp.robots)
      if (r.isTransporter())
      {
        if (r.unSafe)
          --r.unSafe;
        for (auto &f : mp.foes)
          if (r.p.dis(f.pos) <= SAFE_DIS)
          {
            double angle = atan2(f.velocity.y, f.velocity.x) - f.pos.angleTo(r.p);
            if (abs(normAngle(angle)) <= PI * 0.3)
              r.unSafe = 50;
          }
      }
      else
...
void updHisInfo()
{
  ...
  for (auto &r : mp.robots)
  {
    ...
    bool isNear = false;
    for (auto &f : mp.foes)
      if (f.pos.dis(r.p) <= r.radius + f.radius + 0.05)
      {
        double angle = normAngle(atan2(f.velocity.y, f.velocity.x) - f.pos.angleTo(r.p));
        if (abs(angle) <= 1)
          isNear = true;
      }
    r.updIsStop(isNear);
...
void fixState()
{
  for (auto &r : mp.robots)
    ...
    {
      r.state = 2;
      if (r.sell >= 0 && !(mp.plats[r.sell].needBuy >> r.item & 1))
        r.buy = r.sell = -1;
      r.fresh();
    }
  memset(num456, 0, sizeof(num456));
  for (auto &p : mp.plats)
  {
    p.willBuy = 0;
    p.willSellNum = 0;
    ...
    {
      int x = p.num + (p.prodTime >= 0) + ((p.willBuy | p.alreadyBuy) == p.needBuy);
      num456[p.type] += x;
    }
    if (p.type == 7 && !(p.unSafe && p.rushAble <= 1))
    {
      for (int type = 4; type <= 6; ++type)
        if (p.alreadyBuy >> type & 1)
          ++num456[p.type];
    }
  }
}
