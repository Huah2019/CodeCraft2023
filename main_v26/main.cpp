...
bool canMake7Foe;
vector<vector<int>> foePlatsOnBeltline7; // 敌人7号生产线上的工作台计数
// 全局区 end
...
void init()
{
  ...
  // for (auto &p : mp.plats)
  //   dbg("MP: ", p.id, p.rushAble);
  // for (auto &p : mpFoe.plats)
  //   dbg("MPFOE: ", p.id, p.rushAble);
    ...
    if (r.isAttacker())
    {
      ...
      else
      {
        ...
        for (int id = 0; id < mp.foes.size(); ++id)
        {
          ...
          if (std::abs(angle) > PI / 2 && dis > 2 || disToPlat > 1.3)
            continue;
          if (dis < mndis)
          {
            mndis = dis;
...
void processRadarInfo()
{
  ...
  for (int i = 0; i < mp.aliveFoes.size(); i++)
  {
    for (auto &p : mp.plats)
    {
      if (mp.aliveFoes[i].pos.dis(p.p) <= 0.1 && mp.aliveFoes[i].velocity.len2() < 0.05)
      {
