...
struct AStar {
  ...
  D calPenalty(Pt a) {
    if (!Map::instance().role) return 0;
    D res = 0;
    for (int i : robot.foeNeigbors) {
      Pt v = Map::instance().aliveFoes[i].pos - a;
      D d2 = v.len2();
      if (d2 < 1e-8) {
        return 1e9;
      }
      res += 25.0 / d2 + 15.0 / std::sqrt(d2);
    }
    return res;
  }
    ...
    if (!flag) {
      ...
      for (int x : ns) {
        if (check(robot.p, mp.nodes[x].p, robot.radius)) {
          Pt v = mp.nodes[x].p - robot.p;
          D dist = v.len();
          if (robot.unSafe > 40) {
            dist += std::abs(normAngle(std::atan2(v.y, v.x) - robot.angle)) /
                  MAX_ROTATE_SPEED * MAX_FORWARD_SPEED[mp.role] * 2;
          }
      ...
      if (path.empty()) {
        ...
        //   for (int x : ns) {
        //     Logger::instance().write(mp.nodes[x].p, check(robot.p,
        //     mp.nodes[x].p, robot.radius));
