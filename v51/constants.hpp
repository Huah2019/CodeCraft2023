...
constexpr D OTHER_EXR = -0.03;
constexpr D ROBOT_NEIGHBOR_HORIZON = 5.0;
constexpr D FOE_NEIGHBOR_HORIZON = 20.0;
constexpr D OBST_NEIGHBOR_HORIZON = 10.0;
constexpr D IGNORE_DIST = 0.4;  // 如果敌人距离工作台小于这个距离
constexpr D IGNORE_SPEED = 0.2; // 且小于这个速度，则忽略该敌人

constexpr D WAIT_DIST_LIMIT = 2; // 两个参数值应该比较接近
constexpr D RUSH_DIST_LIMIT = 2; // 两个参数值应该比较接近

constexpr D SAFE_DIS = 2;

constexpr int GET_NEXT_TO_NEIGHBOR_RANGE = 4;
...
inline D calAngCost(D cosA, int role)
{
  if (cosA >= 1)
    return 0;
  if (cosA <= -1)
