...
{
  for (Robot &r : mp.robots)
    if (r.isTransporter() && r.state == 1 && r.buy >= 0 && r.pid == r.buy)
    {
      bool flag = true;
      ...
      double eTime1 = mp.plats[r.buy].p.dis(mp.plats[r.sell].p) /
...
void giveUpTask(vector<Robot> &robots, vector<int> &num)
{
  auto delTask = [&](Robot &r)
  {
    if (r.sell >= 0)
    {
      auto &splat = mp.plats[r.sell];
  ...
  for (auto &r : robots)
    if (r.isTransporter() && r.state == 2)
    {
      ...
      else
      {
        if (r.sell >= 0)
        {
          auto &p = mp.plats[r.sell];
          if (!p.isSafe && !p.rushAble)
            delTask(r);
          else
          {
            if (r.itemTLE <= (mp.role == 0 ? -300 : -200))
              delTask(r);
          }
        }
        else
...
void swithTask(vector<Robot> &robots, vector<int> &num)
{
  auto swithTarget = [&](Robot &r)
  {
    ...
    for (auto &splat : mp.plats)
    {
      if (splat.canBuy(r.item))
      {
        ...
        double Score = BUY_PRICE[r.item] / (5 + dis);
        int g = 401 - min(400, frameID - splat.lastUnsafe);
        ...
        if (Score > mxScore && evalTime <= FRAME_LIMIT)
        {
          mxScore = Score;
          r.sell = splat.id;
          r.itemTLE = dis / MAX_FORWARD_SPEED[mp.role] * FPS;
        }
      }
    }
    if (r.sell >= 0)
          ...
          for (int area : bplat.validAreas)
          {
            ...
            for (Platform &splat : mp.plats)
            {
              if (splat.canBuy(bplat.sell))
              {
                ...
                if (bplat.type == 7 && edis1 <= 1) // 防止经过7工作台不卖7
                  baseCof += 50;
                if (canMake7 && bplat.type >= 4 && bplat.type <= 6 && splat.type == 7 && edis1 <= 1)
                  baseCof += 50;
                if (canMake7 && splat.type >= 4 && splat.type <= 6 && num456[splat.type] == minNum456 && sellDisTo7[splat.id] <= 1e9 - 100)
                  baseCof += 6.6;
