...
{
  for (auto &r : robots)
    if (r.state == 2 && (r.isStop || r.sell != -1 && mp.plats[r.sell].unSafe && !mp.plats[r.sell].rushAble))
    {
      if (r.sell >= 0)
      {
        auto &splat = mp.plats[r.sell];
...
void updHisInfo()
{
  ...
  for (auto &r : mp.robots)
  {
    ...
    bool isNear = false;
    for (auto &f : mp.foes)
      if (f.pos.dis(r.p) <= r.radius + f.radius + 0.1)
      {
        double angle = normAngle(atan2(f.pos.y, f.pos.x) - f.pos.angleTo(r.p));
        if (abs(angle) <= 1)
          isNear = true;
      }
    r.updIsStop(isNear);
    // if (r.isTransporter())
    //   dbg("STP", r.id, r.isStop, isNear, mp.foes.size(), r.v.len());
  }
