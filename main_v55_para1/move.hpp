...
inline void moveRobots() {
  ...
  for (Robot &rob : robots) {
    ...
    if (target >= 0 &&
        rob.p.dis(mp.plats[target].p) <= mp.plats[target].getWaitDist() &&
  ...
  for (Robot &rob : robots) {
    ...
    if (rob.isTransporter()) {
      if (rob.isStop > 0) {
        ...
        for (auto &f : mp.foes) {
          Pt v = f.pos - rob.p;
          if (v.len() <= RUSH_DIST_LIMIT &&
              std::fabs(normAngle(std::atan2(v.y, v.x) - rob.angle)) <=
                  PI / 4) {
            rob.lineSpeed = 100;
          }
        }
